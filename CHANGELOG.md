...
- Removed `Layered::Assistant.scope` configuration block. Engine records are now scoped to the signed-in user via a polymorphic `owner` association on every owner-bearing model (`Assistant`, `Conversation`, `Provider`, `Persona`, `Skill`), with reads filtered through `Model.owned_by(current_owner)` in controllers; when there is no signed-in user, `owned_by(nil)` returns no records rather than exposing unowned ones. `current_owner` defaults to `l_ui_current_user` and is the single override point for hosts that want a different ownership boundary (e.g. an organisation) - hosts that previously customised the scope block should override it instead
