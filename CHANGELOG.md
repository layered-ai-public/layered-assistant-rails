...
- Two provider behaviours do not fit the managed form and stay local: creating a provider still seeds its models from the catalogue, through a controller that overrides `create` alone, and the new-provider template picker keeps an ejected `new` template. The choice of whether to seed a provider's models is now a select rather than a radio pair
